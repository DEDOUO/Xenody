    ...
    public static void ConvertExcelToJson()
    {
        try
        {
            using (XLWorkbook workbook = new XLWorkbook(excelFilePath))
            {
                ...
                var colorStartTIndex = GetColumnIndex(colorHeaderRow, "startT");
                var colorEndTIndex = GetColumnIndex(colorHeaderRow, "endT");
                var startLcolorIndex = GetColumnIndex(colorHeaderRow, "StartLcolor");
                var startUcolorIndex = GetColumnIndex(colorHeaderRow, "StartUcolor");
                var endLcolorIndex = GetColumnIndex(colorHeaderRow, "EndLcolor");
                var endUcolorIndex = GetColumnIndex(colorHeaderRow, "EndUcolor");
                ...
                foreach (var row in colorDataRows)
                {
                    ...
                    string startLcolor = ParseStringValue(row.Cell(startLcolorIndex));
                    if (!string.IsNullOrEmpty(startLcolor)) colorRow["StartLcolor"] = startLcolor;

                    string startUcolor = ParseStringValue(row.Cell(startUcolorIndex));
                    if (!string.IsNullOrEmpty(startUcolor)) colorRow["StartUcolor"] = startUcolor;

                    string endLcolor = ParseStringValue(row.Cell(endLcolorIndex));
                    if (!string.IsNullOrEmpty(endLcolor)) colorRow["EndLcolor"] = endLcolor;

                    string endUcolor = ParseStringValue(row.Cell(endUcolorIndex));
                    if (!string.IsNullOrEmpty(endUcolor)) colorRow["EndUcolor"] = endUcolor;
                ...
                // ----------------------
                // 处理合并后的 tap 数据（核心修改部分）
                // 包含原 tap、slide、flick 的数据，新增 Slide 和 Dir 列
                // ----------------------
                IXLWorksheet tapSheet = workbook.Worksheet("tap");
                List<Dictionary<string, object>> tapData = new List<Dictionary<string, object>>();
                var tapHeaderRow = tapSheet.FirstRowUsed();
                ...
                var tapSizeIndex = GetColumnIndex(tapHeaderRow, "Size");
                var tapPidIndex = GetColumnIndex(tapHeaderRow, "Pid");
                var tapSlideIndex = GetColumnIndex(tapHeaderRow, "Slide"); // 新增：判断是否为 slide
                var tapDirIndex = GetColumnIndex(tapHeaderRow, "Dir");     // 新增：划动方向
                ...
                foreach (var row in tapDataRows)
                {
                    ...
                    // 新增字段：Slide（是否为 slide，1 是 / 0 否）
                    int? slideFlag = ParseIntValue(row.Cell(tapSlideIndex), "tap");
                    if (slideFlag.HasValue) tap["Slide"] = slideFlag.Value;

                    // 新增字段：Dir（划动方向，仅 flick 有值）
                    string dir = ParseStringValue(row.Cell(tapDirIndex));
                    if (!string.IsNullOrEmpty(dir)) tap["Dir"] = dir;
                ...
                // 获取列索引（新增 Slide 列，调整 startDir 和 endDir 的作用范围）
                var holdStartTIndex = GetColumnIndex(holdHeaderRow, "startT");
                var holdStartXMinIndex = GetColumnIndex(holdHeaderRow, "startXMin");
                var holdStartXMaxIndex = GetColumnIndex(holdHeaderRow, "startXMax");
                var holdEndTIndex = GetColumnIndex(holdHeaderRow, "endT");
                ...
                // 关键调整：startDir 和 endDir 是 Hold 级别的属性（每个 Hold 唯一）
                var holdStartDirIndex = GetColumnIndex(holdHeaderRow, "startDir");
                var holdEndDirIndex = GetColumnIndex(holdHeaderRow, "endDir");
                // 新增：Slide 属性（判断 Hold 是否为可滑动判定）
                var holdSlideIndex = GetColumnIndex(holdHeaderRow, "Slide");

                var holdDataRows = holdSheet.RowsUsed().Skip(1);
                var validHoldRows = holdDataRows.Where(row => !string.IsNullOrEmpty(row.Cell(holdIdIndex).GetValue<string>()?.Trim()));

                // 按 holdId 分组处理（每个组对应一个 Hold 实例）
                foreach (var group in validHoldRows.GroupBy(row => ParseIntValue(row.Cell(holdIdIndex), "hold")))
                {
                    if (group.Key.HasValue)
                    {
                        ...
                        // 获取当前 Hold 组的第一行（用于提取 Hold 级别的属性）
                        var firstRowInGroup = group.First();

                        // 1. Hold 级别的基础属性
                        int? pid = ParseIntValue(firstRowInGroup.Cell(holdPidIndex), "hold");
                        if (pid.HasValue) hold["Pid"] = pid.Value;

                        hold["id"] = group.Key.Value;

                        // 新增：Slide 属性（是否为可滑动判定的 hold，1 是 / 0 否）
                        int? slideFlag = ParseIntValue(firstRowInGroup.Cell(holdSlideIndex), "hold");
                        if (slideFlag.HasValue) hold["Slide"] = slideFlag.Value;

                        // 调整：startDir 和 endDir 为 Hold 级别属性（取组内第一行的值）
                        double? startDir = ParseDoubleValue(firstRowInGroup.Cell(holdStartDirIndex), 3);
                        if (startDir.HasValue) hold["startDir"] = startDir.Value;

                        double? endDir = ParseDoubleValue(firstRowInGroup.Cell(holdEndDirIndex), 3);
                        if (endDir.HasValue) hold["endDir"] = endDir.Value;

                        // 2. 处理子 Hold 列表（SubHold）
                        List<Dictionary<string, object>> subData = new List<Dictionary<string, object>>();
                        foreach (var row in group)
                        {
                            Dictionary<string, object> subRow = new Dictionary<string, object>();
