...
public static class SongAndChartData
{
    ...
    //                        subRow["Rad"] = ParseDoubleValue(row.Cell(RadIndex), 3);
    //                        subRow["Angle"] = ParseDoubleValue(row.Cell(AngleIndex), 3);
    //                        subRow["Rot"] = ParseDoubleValue(row.Cell(RotIndex), 3);
    //                        subData.Add(subRow);
    //                    }
    //                    star["sub"] = subData;
    //                    starData.Add(star);
        ...
        try
        {
            using (XLWorkbook workbook = new XLWorkbook(excelFilePath))
            {
                ...
                foreach (var row in colorDataRows)
                {
                    ...
                    string lcolor = ParseStringValue(row.Cell(colorLcolorIndex));
                    if (!string.IsNullOrEmpty(lcolor)) colorRow["Lcolor"] = lcolor;

                    string ucolor = ParseStringValue(row.Cell(colorUcolorIndex));
                    if (!string.IsNullOrEmpty(ucolor)) colorRow["Ucolor"] = ucolor;
